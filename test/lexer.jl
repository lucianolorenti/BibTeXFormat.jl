MockLexer = LatexLexer( tokens = [
        ("chars", "mock"),
        ("unknown", ".")
       ])

MockIncrementalDecoder = LatexIncrementalDecoder(tokens = [
        ("chars", "mock"),
        ("unknown", "."),
       ])
@testset "Latex Lexer Test" begin
    lexer = Lexer(errors="strict")
    function lex_it(self::Lexer, latex_code, latex_tokens, final=false):
        if !(self.lexer.binary_mode)
            latex_code = decode(latex_code, "ascii")
            latex_tokens = [decode(token, "ascii") for token in latex_tokens]
        end
        tokens = get_raw_tokens(self, latex_code, final=final)
        @test  [token.text for token in tokens] ==  latex_tokens
    end
    @testset "null" begin
        lex_it(lexer,"", [], final=true)
    end
    @testset "hello" begin
        lex_it(lexer,
            "hello!  [#1] This \\is\\   \\^ a \ntest.\n    \nHey.\n\n\# x \#x",
            split("""h|e|l|l|o|!| | |[|#1|]| |T|h|i|s| |\is|\ | | |\^| |a| '
            b'|\n|t|e|s|t|.|\n| | | | |\n|H|e|y|.|\n|\n'
            br'|\#| |x| |\#|x""",'|'),
            final=true)
    end
    @testset "comment" begin
        lex_it(lexer,
            "test% some comment\ntest",
            split("'t|e|s|t|% some comment|\n|t|e|s|t",'|'),
            final=true)
    end
    @testset "commend_newline" begin
        lex_it(lexer,
            "test% some comment\n\ntest",
            split("t|e|s|t|% some comment|\n|\n|t|e|s|t",'|'),
            final=true
    ...
    @testset "control_whitespace" begin
        lex_it(lexer,
            "'\\hello   \\world   ",
            split("\\hello| | | |\\world| | | ",'|'),
            final=true
    ...
    @testset "controlx_whitespace" begin
        lex_it(lexer,
            "\\#    \\&   ",
            split("\\#| | | | |\\&| | | ",'|'),
            final=true
