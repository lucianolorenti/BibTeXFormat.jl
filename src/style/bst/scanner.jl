...
function update_lineno(self::Scanner, value)
    matches = collect(m.match for m in eachmatch(NEWLINE[1], value))
    num_newlines = length(matches)
...
function required(self::Scanner, patterns, description=nothing; allow_eof=false)
    token = get_token(self, patterns, allow_eof=allow_eof)
    if token == nothing
